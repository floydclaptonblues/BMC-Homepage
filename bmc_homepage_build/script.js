...
(function restoreHomepage(){
  ...
  const bandBase = "https://floydclaptonblues.github.io/Balcony-Music-Club-PWA/assets/bands/bmc-band-assets/assets/bands/";
  const bandPhotos = [
    { name: "Tropical Weather", src: bandBase + "Tropical%20Weather.png" },
    { name: "Josh Benitez Band", src: bandBase + "Josh%20Benitez%20Band%20Sunday.png" },
    { name: "Jam Brass Band", src: bandBase + "Jam%20Brass%20Band%20Thursday%20%281%29.jpg" },
    { name: "Leroy Marshall", src: bandBase + "Leroy%20Marshal.jpg" },
    { name: "Jon Roniger", src: bandBase + "JON%20RONIGER.jpg" },
    { name: "Fleurtations", src: bandBase + "FLEURTATIONS.jpg" }
  ];
  ...
  css.textContent = `
    ...
    .stage-photo.bmc-stage-band-wire{height:auto!important;min-height:118px!important;background-image:linear-gradient(180deg,rgba(18,7,38,.72),rgba(18,7,38,.9))!important;background-size:cover!important;background-position:center!important;padding:6px!important;overflow:hidden!important}
    .bmc-home-band-strip{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:6px!important;width:100%!important}
    .bmc-home-band-tile{display:block!important;min-width:0!important;overflow:hidden!important;border:1px solid rgba(255,210,91,.55)!important;background:rgba(9,2,22,.62)!important;text-decoration:none!important;color:var(--cream)!important}
    .bmc-home-band-tile img{display:block!important;width:100%!important;height:62px!important;object-fit:cover!important;background:#120728!important;image-rendering:auto!important}
    .bmc-home-band-tile span{display:block!important;padding:4px 5px!important;color:var(--gold)!important;font-size:8px!important;font-weight:950!important;letter-spacing:.03em!important;text-transform:uppercase!important;line-height:1.1!important;text-shadow:0 1px 0 #000!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}
  ...
  function wireBandPhotos(){
    const stage = document.querySelector(".stage-photo");
    if (!stage || stage.dataset.bandPhotoWire === "true") return;
    stage.dataset.bandPhotoWire = "true";
    stage.classList.add("bmc-stage-band-wire");
    stage.setAttribute("aria-label", "Featured Balcony Music Club July artist photos");
    stage.textContent = "";
    const strip = document.createElement("div");
    strip.className = "bmc-home-band-strip";
    bandPhotos.forEach((band) => {
      const tile = document.createElement("a");
      tile.className = "bmc-home-band-tile";
      setLink(tile, links.shows, "_top");
      const img = document.createElement("img");
      img.src = band.src;
      img.alt = band.name + " promo photo";
      img.loading = "lazy";
      img.decoding = "async";
  ...
  placeBmcLiveFirst();
  wireBandPhotos();
  setTimeout(wireBandPhotos, 250);
  setTimeout(wireBandPhotos, 1000);
...
(function mountHomepageJazzyUfoBouncer(){
  if (document.getElementById("bmc-homepage-ufo-bouncer")) return;

  const style = document.createElement("style");
  style.id = "bmc-homepage-ufo-bouncer-style";
  style.textContent = `
    .site-header,main#home,.site-footer{position:relative;z-index:2}
    #bmc-homepage-ufo-bouncer{
      position:fixed;
      left:0;
      top:0;
      z-index:1;
      width:clamp(132px,15vw,260px);
      pointer-events:none;
      opacity:.78;
      filter:drop-shadow(0 0 12px rgba(94,230,255,.42)) drop-shadow(0 0 22px rgba(255,216,87,.25));
      transform:translate3d(calc(100vw - 300px),12vh,0);
      animation:bmcHomepageUfoZigzag 28s linear infinite alternate;
      will-change:transform;
    }
    #bmc-homepage-ufo-bouncer img{display:block;width:100%;height:auto;user-select:none;-webkit-user-drag:none}
    @keyframes bmcHomepageUfoZigzag{
      0%{transform:translate3d(calc(100vw - min(300px,31vw)),10vh,0) scaleX(1)}
      18%{transform:translate3d(2vw,22vh,0) scaleX(-1)}
      35%{transform:translate3d(calc(100vw - min(320px,33vw)),36vh,0) scaleX(1)}
      52%{transform:translate3d(5vw,50vh,0) scaleX(-1)}
      70%{transform:translate3d(calc(100vw - min(300px,31vw)),63vh,0) scaleX(1)}
      86%{transform:translate3d(3vw,76vh,0) scaleX(-1)}
      100%{transform:translate3d(calc(100vw - min(330px,34vw)),82vh,0) scaleX(1)}
    }
    @media(max-width:760px){#bmc-homepage-ufo-bouncer{width:clamp(82px,22vw,142px);opacity:.66;animation-duration:22s}@keyframes bmcHomepageUfoZigzag{0%{transform:translate3d(calc(100vw - 138px),8vh,0) scaleX(1)}20%{transform:translate3d(2vw,22vh,0) scaleX(-1)}40%{transform:translate3d(calc(100vw - 138px),38vh,0) scaleX(1)}60%{transform:translate3d(4vw,54vh,0) scaleX(-1)}80%{transform:translate3d(calc(100vw - 138px),70vh,0) scaleX(1)}100%{transform:translate3d(3vw,84vh,0) scaleX(-1)}}}
    @media(prefers-reduced-motion:reduce){#bmc-homepage-ufo-bouncer{animation:none;transform:translate3d(calc(100vw - 180px),16vh,0);opacity:.42}}
  `;
  document.head.appendChild(style);

  const ufo = document.createElement("div");
  ufo.id = "bmc-homepage-ufo-bouncer";
  ufo.setAttribute("aria-hidden", "true");
  const img = document.createElement("img");
  img.src = "https://floydclaptonblues.github.io/Balcony-Music-Club-PWA/assets/venue/bmc_jazzycat_ufo_360_rotation.gif?v=20260705-homepage-zigzag";
  img.alt = "";
  img.decoding = "async";
  img.loading = "lazy";
  ufo.appendChild(img);

...
(function restoreHomepage(){
  ...
  css.textContent = `
    ...
    .stage-photo{height:118px!important;background-image:linear-gradient(90deg,rgba(18,7,38,.10),rgba(18,7,38,.26)),url("assets/gallery-4.jpg")!important;background-size:cover!important;background-position:center 48%!important;image-rendering:auto!important;overflow:hidden!important}
  ...
  function restoreStagePainting(){
    const stage = document.querySelector(".stage-photo");
    if (!stage) return;
    stage.dataset.bandPhotoWire = "false";
    stage.classList.remove("bmc-stage-band-wire");
    stage.setAttribute("aria-label", "Balcony Music Club stage mural painting");
    stage.textContent = "";
    stage.querySelectorAll("*").forEach((node) => node.remove());
  }

  setLink(document.querySelector(".brand"), links.home);
  placeBmcLiveFirst();
  restoreStagePainting();
  setTimeout(restoreStagePainting, 100);
  setTimeout(restoreStagePainting, 400);
  setTimeout(restoreStagePainting, 1000);
